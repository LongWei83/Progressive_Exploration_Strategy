...
import InitX
import History
from MultiObjectiveEvaluator import MultiObjectiveEvaluator
...
class Group(object):
    def __init__(self, personal_num, max_iter, dimension, init_x, xL, xU):
        ...
        self.gBest_value = [0.0,0.0,100]                  # group best fitness
    ...
    def moo_de_single_step(self,his_dir,tmp_dir,localTask_manager,localtaskVBApath,time_list):
        # 单步PSO算法
        pre_gBest_value = [0.0,0.0,100]
        pre_gBest_value[0] = self.gBest_value[0]
        pre_gBest_value[1] = self.gBest_value[1]
        pre_gBest_value[2] = self.gBest_value[2]
        MultiObjectiveEvaluator.calculate_all_fitness(self.personals)
        for personal in self.personals:
            personal.moo_de_single_step()
            # 更新全局最佳位置和适应度值
            if (personal.pBest_value[0] >= self.gBest_value[0]) and (personal.pBest_value[1] >= self.gBest_value[1]) and (abs(personal.pBest_value[2]-5.71) <= abs(self.gBest_value[2]-5.71)):
                self.gBest_value[0] = personal.pBest_value[0]
                self.gBest_value[1] = personal.pBest_value[1]
                self.gBest_value[2] = personal.pBest_value[2]
                self.gBest = np.copy(personal.pBest)
                self.gBest_ShuntImpedance = personal.pBest_shuntImpedance
                self.gBest_Freq = personal.pBest_freq
                self.gBest_QFactor = personal.pBest_qFactor
                self.gBest_id = personal.id
            ...
            #print(personal.pBest_value) #个体最佳适应度
        #防止所有粒子进入局部最优解
        if pre_gBest_value[0] == self.gBest_value[0] and pre_gBest_value[1] == self.gBest_value[1] and pre_gBest_value[2] == self.gBest_value[2]:
             self.local_trap_cnt = self.local_trap_cnt + 1
        else:
             self.local_trap_cnt = 0
        ...
        f.write(str(self.gBest_ShuntImpedance)+"\n")
        f.write(str(self.gBest_Freq)+"\n")
        f.write(str(self.gBest_QFactor)+"\n")
        f.close()

        for personal in self.personals:
            # Add DE single step
            # 按照 rank 升序，crowding_distance 降序排序
            sorted_personals = sorted(
                self.personals,
                key=lambda p: (p.moo_fitness[0], p.moo_fitness[1])  # (rank, -crowding_distance)
            )

            # 取前10个个体，并排除当前 personal
            available_personals = [p for p in sorted_personals[:10] if p != personal]

            # 若不足两个可选个体，则退回整个种群中挑选（防止 sample 报错）
            if len(available_personals) >= 2:
            ...
            if trial_RoverQ > 20:
                 if (trial_ShuntImpedance > personal.shuntImpedance) and (trial_QFactor > personal.qFactor) and (abs(trial_Frequency-5.71) <= abs(personal.frequency-5.71)):
                    personal.x = U
                    personal.shuntImpedance = trial_ShuntImpedance
                    personal.qFactor = trial_QFactor
                    personal.frequency = trial_Frequency
