...
cstexepath = "D:\cst\CST DESIGN ENVIRONMENT.exe"
...
pso_group = Group.Group(personal_num, max_iter, dimension, x, xL, xU)

pso_group.create_particles()
...
while True:
    task_flag_files = []
    #发布本地CST处理任务文件
    if pso_group.distribute_tasks(tmp_dir):
        break
    else:
        print("next iter")
    ...
    #读取CST仿真结果
    pso_group.read_results(tmp_dir)
    ...
    # 单步PSO算法
    pso_group.pso_single_step(his_dir,time_list)
