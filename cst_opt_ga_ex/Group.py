...
class Group(object):
    ...
    def create_particles(self):
        ...
        if self.local_trap_cnt >= 3:
            for tmp_personal, personal in zip(tmp_personals, self.personals):
                ...
                personal.pBest = tmp_personal.pBest
                personal.pBest_value = tmp_personal.pBest_value
    ...
    def ga_single_step(self, his_dir, tmp_dir, localTask_manager, localtaskVBApath, time_list):
        pre_gBest_value = self.gBest_value

        # Step 1: 选择（Tournament Selection）
        def tournament_selection(k=2):
            selected = random.sample(self.personals, k)
            selected.sort(key=lambda p: p.fitness, reverse=True)
            return selected[0]
        ...
        new_population = []
        new_id = 0
        while len(new_population) < len(self.personals):
            ...
            for i in range(self.dimension):
                if random.random() < 0.5:
                    child_x.append(parent1.x[i])
                else:
                    child_x.append(parent2.x[i])

            # Step 4: 变异
            for i in range(self.dimension):
                if random.random() < 0.1:  # 变异概率
                    lower = self.gBest[i] - (parent1.xU[i] - parent1.xL[i])*0.05*self.scale
                    upper = self.gBest[i] + (parent1.xU[i] - parent1.xL[i])*0.05*self.scale

                    if lower < parent1.xL[i]:
                         lower = parent1.xL[i]

                    if upper > parent1.xU[i]:
                         upper = parent1.xU[i]
                    child_x[i] = round(random.uniform(lower, upper), 4)

            # 将 trial vector 写入临时文件用于 CST 模拟计算
            trial_task_path = os.path.abspath(os.path.join(tmp_dir, "trial_task.txt"))
            with open(trial_task_path, "w") as f:
                for val in child_x:
                    f.write(f"{val}\n")
            ...
            # Step 5: 适应度评估
            trial_fitness = self.personals[0].trial_fitness(trial_RoverQ, trial_Frequency, trial_ShuntImpedance)

            # Step 6: 创建新个体
            new_person = copy.deepcopy(self.personals[0])
            new_person.x = child_x
            new_person.fitness = trial_fitness
            new_person.frequency = trial_Frequency
            new_person.qFactor = trial_QFactor
            new_person.roverQ = trial_RoverQ
            new_person.shuntImpedance = trial_ShuntImpedance
            new_person.recalc = True
        ...
        # Step 8: 替换种群
        for org_personal, new_personal  in zip(self.personals, new_population):
             new_personal.x_List = org_personal.x_List
             new_personal.fitness_List = org_personal.fitness_List
             new_personal.shuntImpedance_List = org_personal.shuntImpedance_List
             new_personal.frequency_List = org_personal.frequency_List
             new_personal.rOverQ_List = org_personal.rOverQ_List
             new_personal.pBest = org_personal.pBest
             new_personal.pBest_value = org_personal.pBest_value
             
        self.personals = new_population
